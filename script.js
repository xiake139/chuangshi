// ==================== 配置 (Back4App) ====================
const APP_ID = 'KFnXgQymPOONJteaW81UcwgHFxnnkKpL9WJuUbij';
const JS_KEY = 'BsaVOQI2fXHRf4yCtJ7PFC8Xq99KokIlL6QIm0Ya';
const SERVER_URL = 'https://parseapi.back4app.com';

// 检查 Parse 是否已加载
if (typeof Parse === 'undefined') {
    alert('错误：Parse SDK 未加载，请检查网络或 CDN 地址');
} else {
    Parse.initialize(APP_ID, JS_KEY);
    Parse.serverURL = SERVER_URL;
    console.log('Parse initialized with:', Parse.applicationId, Parse.javaScriptKey);
}
...
let userId = null; // 当前登录用户 ID，从 Parse 获取
...
async function loadGameData() {
    ...
    // 查询当前用户的 GameData
    const query = new Parse.Query('GameData');
    query.equalTo('user', currentUser);
    const gameData = await query.first();
...
async function saveCharacter() {
    ...
    const query = new Parse.Query('GameData');
    query.equalTo('user', currentUser);
    let gameData = await query.first();

    if (!gameData) {
        gameData = new Parse.Object('GameData');
        gameData.set('user', currentUser);
    }
    ...
    gameData.set('player', player);
    gameData.set('inventory', inventory);
    gameData.set('extra', extra);
...
function renderAuthScreen() {
    return `
        <div class="panel" style="text-align:center;">
            ...
            <!-- 调试信息区域 -->
            <div id="debugInfo" style="margin-top:20px; background:#0b0f1a; padding:10px; border-radius:10px; color:#aaa; font-size:12px; text-align:left;"></div>
        </div>
    `;
}
...
function attachAuthEvents() {
    ...
    if (debugDiv) {
        debugDiv.innerHTML = `
            <div>Parse 初始化状态：</div>
            <div>App ID: ${Parse.applicationId || '未设置'}</div>
            <div>JS Key: ${Parse.javaScriptKey || '未设置'}</div>
            <div>Server URL: ${Parse.serverURL || '未设置'}</div>
        `;
    }
